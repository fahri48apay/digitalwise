...
router.get('/threads', async (req: Request, res: Response) => {
  try {
    ...
    let query: any = db.collection('forum_threads');
...
router.get('/threads/:id', async (req: Request, res: Response) => {
  try {
    const threadDoc = await db.collection('forum_threads').doc(req.params.id).get();
    ...
    const commentsSnapshot = await db.collection('forum_threads')
      .doc(req.params.id)
      .collection('comments')
    ...
    res.json({
      success: true,
      data: { id: threadDoc.id, ...threadDoc.data(), comments }
    });
  } catch (error) {
...
router.post('/threads', async (req: Request, res: Response) => {
  try {
    ...
    thread.lastActivityAt = admin.firestore.FieldValue.serverTimestamp();
    thread.createdAt = admin.firestore.FieldValue.serverTimestamp();
    thread.updatedAt = admin.firestore.FieldValue.serverTimestamp();
    
    const docRef = await db.collection('forum_threads').add(thread);
    
    res.status(201).json({ success: true, data: { id: docRef.id, ...thread } });
  } catch (error) {
...
router.put('/threads/:id', async (req: Request, res: Response) => {
  try {
    ...
    const doc = await db.collection('forum_threads').doc(req.params.id).get();
    if (!doc.exists) {
      res.status(404).json({ success: false, error: 'Thread not found' });
      return;
    }
    ...
    await db.collection('forum_threads').doc(req.params.id).update(updates);
...
router.delete('/threads/:id', async (req: Request, res: Response) => {
  try {
    ...
    await db.collection('forum_threads').doc(req.params.id).delete();
...
router.post('/threads/:id/like', async (req: Request, res: Response) => {
  try {
    ...
    const likeDoc = await db.collection('forum_threads')
      .doc(req.params.id)
      .collection('likes')
      .doc(userId)
      .get();
    ...
    if (likeDoc.exists) {
      await db.collection('forum_threads')
        .doc(req.params.id)
        .collection('likes')
        .doc(userId)
        .delete();
      
      await db.collection('forum_threads').doc(req.params.id).update({
      ...
      await db.collection('forum_threads')
        ...
        .collection('likes')
        .doc(userId)
        .set({ userId, createdAt: admin.firestore.FieldValue.serverTimestamp() });
...
router.post('/threads/:id/comments', async (req: Request, res: Response) => {
  try {
    ...
    comment.updatedAt = admin.firestore.FieldValue.serverTimestamp();
    
    const docRef = await db.collection('forum_threads')
      .doc(req.params.id)
      .collection('comments')
      .add(comment);
...
router.put('/comments/:id', async (req: Request, res: Response) => {
  try {
    ...
    const doc = await db.collection('forum_threads')
      .doc(threadId)
      .collection('comments')
      .doc(req.params.id)
      .get();
